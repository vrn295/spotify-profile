...
const ProfileMostPlayed = () => {
  ...
  const handleMostPlayedCall = async (
    timeRange: TimeRange = TimeRange.SIX_MONTH
  ) => {
  ...
  return (
    <div className={styles.container}>
      <ProfileLastPlayed
        ...
        cover={
          mostPlayed?.items[0]?.album?.images[0]?.url ||
          "/images/home/nf_cover.jpg"
        }
      ...
      <div className={styles.profile_most_played_container}>
        ...
        <section className={styles.profile_most_played}>
          {(mostPlayed?.items?.slice(1) || [...Array(15)]).map(
            (item, index) => (
              <SongCard
                key={item?.id || index}
                title={item?.name}
                ...
                cover={
                  item?.album?.images[0]?.url || "/images/home/nf_cover.jpg"
                }
